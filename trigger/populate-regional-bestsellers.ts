...
// list_title is added by fetchRegionalList after parsing; everything else
// comes from the shared parser.
type RegionalBook = SharedRegionalBook & { list_title: string | null };
...
function parseRegionalList(
  ...
  const listTitle = getListTitle(region);
  return parseRegionalListShared(content, region, weekDate).map((book) => ({
    ...book,
